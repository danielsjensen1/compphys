...
nleft = Atom(Z, exponents, R1)
nright = Atom(Z, exponents, R2)
h2plus = H2Plus((nleft, nright))
h2plus.variational()
def print_energies():
    for n in range(1, len(exponents)+1):
        approx = h2plus.approx_energy(n)
        exact = h2plus.exact_energy(n)
...
#    plt.plot(r, exact, 'b--', label='STO')
    ...
    plt.legend()
    plt.title('Ground-state Electronic Eigenfunction of H2+ Molecule')
    plt.show()
