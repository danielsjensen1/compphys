...
#        savemat('static.mat', {'S': self.S, 'T': self.T, 'A': self.A, 'Q': self.Q})
    ...
    def two_electron(self, alphas, positions, Spq, Srs):
        alpha_p, alpha_r, alpha_q, alpha_s = alphas
        A = alpha_p + alpha_q
        B = alpha_r + alpha_s
        R_p, R_r, R_q, R_s = positions
        R_A = (alpha_p * R_p + alpha_q * R_q) / A
        R_B = (alpha_r * R_r + alpha_s * R_s) / B
        R_AB = R_A - R_B
        R_AB_sqrd = dot(R_AB, R_AB)
        t = A * B / (A + B) * R_AB_sqrd
